...
class YOLO_TF:
	...
	filewrite_txt = False
	disp_console = True
	weights_file = 'weights/YOLO_car_tiny.ckpt'
	alpha = 0.1
	threshold = 0.1
	iou_threshold = 0.5
	num_class = 1
	...
	def conv_layer(self,idx,inputs,filters,size,stride):
		...
		weight = tf.Variable(tf.truncated_normal([size,size,int(channels),filters], stddev=0.1), name='yolo/conv_'+str(idx)+'/weights')
		biases = tf.Variable(tf.constant(0.1, shape=[filters]), name='yolo/conv_'+str(idx)+'/biases')
	...
	def fc_layer(self,idx,inputs,hiddens,flat = False,linear = False):
		...
		weight = tf.Variable(tf.truncated_normal([dim,hiddens], stddev=0.1), name='yolo/fc_'+str(idx)+'/weights')
		biases = tf.Variable(tf.constant(0.1, shape=[hiddens]), name='yolo/fc_'+str(idx)+'/biases')	
	...
	def interpret_output(self,output):
		...
		class_probs = np.reshape(output[0:121],(11,11,1))
		scales = np.reshape(output[121:363],(11,11,2))
		boxes = np.reshape(output[363:],(11,11,2,4))
		offset = np.transpose(np.reshape(np.array([np.arange(11)]*22),(2,11,11)),(1,2,0))
		
		print scales

		boxes[:,:,:,0] += offset
		boxes[:,:,:,1] += np.transpose(offset,(1,0,2))
		boxes[:,:,:,0:2] = boxes[:,:,:,0:2] / 11.0
