...
{
    //char filename[50];
    //sprintf(filename, "cjy/%d_crop.txt", (int)time(NULL));
    ...
    for(b = 0; b < l.batch; ++b){
        for(c = 0; c < l.c; ++c){
            for(i = 0; i < l.crop_height; ++i){
    		    ...
    		    //fprintf(fp,"h,w,c,v = %d, %d, %d, %f\n",row,col,c,state.input[index]*scale + trans);
		    //fprintf(fp,"%f\n",state.input[index]*scale + trans);
    		    //fflush(fp);
                }
            }
        }
